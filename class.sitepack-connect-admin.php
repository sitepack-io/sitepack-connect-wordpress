...
class SitePackConnectAdmin
{

    const SITEPACK_API_KEY = 'sitepack_api_key';
    const SITEPACK_API_SECRET = 'sitepack_api_secret';
...
//        add_action('admin_init', [$this, 'registerSettings']);
    ...
    {
        add_option(self::SITEPACK_API_KEY, '', '', false);
        add_option(self::SITEPACK_API_SECRET, '', '', false);

        if (empty(get_option(self::SITEPACK_API_KEY, null)) || empty(get_option(self::SITEPACK_API_SECRET, null))) {
            $this->initApiKeys();
        }

        add_filter('plugin_action_links_sitepack-connect/sitepack-connect.php', [$this, 'settingsLink']);
    }

    /**
     * Link sitepack menu pages
     */
    public function adminPages()
    {
        remove_menu_page('sitepack-settings');
        remove_menu_page('options-general.php?page=sitepack-settings');

        add_menu_page(
            __('SitePack dashboard', 'sitepack-connect'),
            'SitePack',
            'manage_options',
            'sitepack-connect',
    ...
    {
        wp_enqueue_style(
            'admin-sitepack-styles',
            plugin_dir_url(SITEPACK_PLUGIN_BASENAME) . '/assets/sitepack_admin.css'
        );
    }

    public function renderDashboardPage()
    {
        require_once(SITEPACK_CONNECT_PLUGIN_DIR . 'views/sitepack_dashboard.php');
    }
    ...
    public function settingsLink($links): array
    {
        ...
        $settings_link = "<a href='" . esc_url($url) . "'>" . __('Settings', 'sitepack-connect') . '</a>';
        array_push($links, $settings_link);

        return $links;
    }

    private function getSvgIcon(bool $baseEncode = true)
    {
    ...
    {
        update_option(
            self::SITEPACK_API_KEY,
            $this->generateKey(self::SITEPACK_API_KEY, 2500)
        );
        update_option(
            self::SITEPACK_API_SECRET,
            $this->generateKey(self::SITEPACK_API_SECRET, 5000)
        );
    }

    private function generateKey(string $input, int $rangeEnd): string
    {
        return md5($input . rand(1, $rangeEnd) . time() . rand(1, $rangeEnd));
    }
